...
app.use(cors({
  origin: function (origin, callback) {
    const allowedOrigins = [
      ...
      // Test VM IP
      'http://20.224.194.131',
      'http://20.224.194.131:80',
      'https://20.224.194.131',
...
// OCR routes - optional, only load if file exists
let ocrRoutes = null;
try {
  ocrRoutes = require('./routes/ocr');
  logger.info('OCR routes loaded successfully');
} catch (error) {
  logger.warn('OCR routes not available', { error: error.message });
}
